        ...
        const fetchData = async (snapshot: any) => {
            ...
            if (messageData) {
                ...
                for (const userId of Object.keys(messageData)) {
                    ...
                    try {
                        const userSnapshot = await get(ref(database, `mobile/users/${userId}`));
                        const snapval = userSnapshot.val();
                        const { email, photoURL, active } = snapval;
                        messageArray.push({ active, photoURL, email, userId, messages: userMessageArray });
                    } catch (error) {
                ...
                </div>
                {
                    !isMounted ?
                        ...
                        <div className="grid grid-cols-12 gap-4 h-full">
                            <div className={`flex flex-col gap-4 col-span-4 ${theme === 'dark' ? ' bg-[#172030]' : 'bg-[#E6E6FA]'} rounded-2xl p-4`}>
                            ...
                            <div className={`flex flex-col gap-4 col-span-8 ${theme === 'dark' ? ' bg-[#172030]' : 'bg-[#E6E6FA]'} rounded-2xl p-4 h-full w-full`}>
